...
export default class WebhooksController {
    ...
    private async getUserNotionConfig(auth: any) {
        ...
        try {
            ...
            if (!user) {
                ...
                console.log('=== FIN GET USER NOTION CONFIG DEBUG ===\n')
     ...
     */
    async testN8nConnection({ request, response, auth }: HttpContext) {
        try {
            console.log('🚀 TEST N8N CONNECTION - Utilisateur connecté:', {
                id: auth.user?.id,
            ...
            // Test simple avec données de test incluant les variables d'environnement
            const testData = {
                source: 'adonis-gmb-test',
                test: true,
                timestamp: new Date().toISOString(),

                // Ajout des variables d'environnement Notion
                notion_config: {
     ...
     */
    async sendBulkToN8n({ request, response, auth }: HttpContext) {
        try {
            console.log('🚀 SEND BULK TO N8N - Utilisateur connecté:', {
                id: auth.user?.id,
                email: auth.user?.email,
                username: auth.user?.username
            })
            ...
            const bulkData = {
                ...
                // Configuration Notion de l'utilisateur
                notion_config: {
                    api_key: notionConfig.apiKey,
                    database_id: notionConfig.databaseId,
                    instance: notionConfig.instance,
                ...
                notion_pages: pages.map((page: any) => ({
                    ...
                    extracted_data: {
                        entreprise:
                            page.properties?.["Nom de l'entreprise"]?.formula?.string || null,
                        etat: page.properties?.['État']?.status?.name || null,
                        referenceurs: page.properties?.['Référenceurs']?.relation || [],
                        spreadsheet_id:
                            page.properties?.['ID Spreadsheet (GMB)']?.formula?.string || null,
                        nombre_posts:
                            page.properties?.['Nombre de posts à générer']?.number || null,
                        mot_cle_objectif:
                            page.properties?.['Mot-clé objectif']?.formula?.string || null,
                ...
                summary: {
                    total_pages: pages.length,
                    pages_with_entreprise: pages.filter(
                        (p: any) => p.properties?.["Nom de l'entreprise"]?.formula?.string
                    ).length,
                    pages_a_generer: pages.filter(
                        (p: any) => p.properties?.['État']?.status?.name === 'À générer'
                    ).length,
                    pages_with_spreadsheet: pages.filter(
                        (p: any) => p.properties?.['ID Spreadsheet (GMB)']?.formula?.string
                    ).length,
                },
            }
    ...
    async sendToN8n({ request, response, auth }: HttpContext) {
        try {
            console.log('🚀 SEND TO N8N - Utilisateur connecté:', {
                id: auth.user?.id,
                email: auth.user?.email,
                username: auth.user?.username
            })
            
            const operationData = request.only([
                'id',
                'title',
                'url',
                'created_time',
            ...
            const optimizedData = {
                ...
                extracted_data: {
                    entreprise:
                        operationData.properties?.["Nom de l'entreprise"]?.formula?.string || null,
                    etat: operationData.properties?.['État']?.status?.name || null,
                    referenceurs: operationData.properties?.['Référenceurs']?.relation || [],
                    spreadsheet_id:
                        operationData.properties?.['ID Spreadsheet (GMB)']?.formula?.string || null,
                    nombre_posts:
                        operationData.properties?.['Nombre de posts à générer']?.number || null,
                    mot_cle_objectif:
                        operationData.properties?.['Mot-clé objectif']?.formula?.string || null,
                    location_id:
                        operationData.properties?.['LocationID (GMB)']?.rollup?.array || [],
